...
function startManagerPolling(token) {
  ...
  managerPoll = setInterval(() => {
    ...
    if (kind === 'manage' && current === token && !document.getElementById('nextTitle')) {
      manager(token, true);
