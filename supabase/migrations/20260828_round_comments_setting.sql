-- Follow-up migration: allow manager to change participant-comments setting when opening a new round.
-- This is intentionally separate because 20260828_participant_comments.sql may already be applied.

-- Remove the current 7-argument version to avoid keeping two active overloads.
drop function if exists public.start_next_round_v04_rpc(text, text, text, numeric, timestamptz, integer, text);

create function public.start_next_round_v04_rpc(
...
begin
  ...
  set round_number = i.round_number + 1,
      ...
      comments_enabled = coalesce(p_comments_enabled, v_initiative.comments_enabled),
      status = 'open',
      closed_at = null
...
revoke all
on function public.start_next_round_v04_rpc(text, text, text, numeric, timestamptz, integer, text, boolean)
from public;
