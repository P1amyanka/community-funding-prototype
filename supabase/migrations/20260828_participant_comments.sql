-- Participant comments for initiative proposals.
-- Comments are enabled by default for all existing and new initiatives.

alter table public.initiatives_v04
  add column if not exists comments_enabled boolean not null default true;

alter table public.proposals_v04
  add column if not exists comment text;

-- Recreate RPCs whose signatures/return types change.
drop function if exists public.create_initiative_v04_rpc(text, text, numeric, timestamptz, integer, text);
...
begin
  ...
  insert into public.initiatives_v04 (
    title, description, target_amount, deadline, expected_participants, payment_details, comments_enabled
  ) values (
...
revoke all on function public.create_initiative_v04_rpc(text, text, numeric, timestamptz, integer, text, boolean) from public;
grant execute on function public.create_initiative_v04_rpc(text, text, numeric, timestamptz, integer, text, boolean) to anon, authenticated;

-- Manager state now exposes whether comments are enabled.
drop function if exists public.get_manager_state_v04_rpc(text);

create function public.get_manager_state_v04_rpc(p_manager_token text)
returns table (
  id uuid,
  title text,
  description text,
...
as $$
  select i.id, i.title, i.description, i.target_amount, i.deadline,
         i.expected_participants, i.payment_details, i.comments_enabled,
         i.status, i.round_number, i.participant_token, i.created_at, i.closed_at
...
-- Manager proposal list includes the optional comment.
drop function if exists public.get_manager_proposals_v04_rpc(text);

create function public.get_manager_proposals_v04_rpc(p_manager_token text)
returns table (
  id uuid,
...
as $$
  select p.id, p.participant_label, p.max_amount, p.recommended_amount,
         p.comment, p.created_at, p.updated_at
  from public.initiatives_v04 i
...
-- Participant state includes comment setting and the participant's own current-round comment.
drop function if exists public.get_participant_state_v04_rpc(text, uuid);

create function public.get_participant_state_v04_rpc(
...
as $$
  select i.title, i.description, i.target_amount, i.deadline,
         ...
         p.participant_label, p.max_amount, p.recommended_amount, p.comment,
         h.feasible, h.gap
  from public.initiatives_v04 i
...
-- Upsert comment together with the amount. The default keeps old cached clients compatible.
drop function if exists public.upsert_proposal_v04_rpc(text, uuid, text, numeric);

create function public.upsert_proposal_v04_rpc(
  p_participant_token text,
...
begin
  ...
  v_comment := case
    when v_initiative.comments_enabled then nullif(trim(coalesce(p_comment, '')), '')
    else null
  end;

  insert into public.proposals_v04 (
    initiative_id, round_number, participant_key, participant_label, max_amount, comment
  ) values (
    v_initiative.id, v_initiative.round_number, p_participant_key,
    trim(p_participant_label), p_max_amount, v_comment
  )
...
revoke all on function public.upsert_proposal_v04_rpc(text, uuid, text, numeric, text) from public;
grant execute on function public.upsert_proposal_v04_rpc(text, uuid, text, numeric, text) to anon, authenticated;

-- Preserve comments in round history so they remain available after a new round starts.
...
begin
  ...
  insert into tmp_v04_alloc (id, participant_label, comment, max_cents, created_at)
  select p.id, p.participant_label, p.comment,
